...
export async function onRequestGet(context) {
  ...
  const todayStr = etParts(new Date()).date;
  ...
  function lastWeekday(year, month, weekday) {
    ...
    while (lastDay.getUTCDay() !== weekday) lastDay.setUTCDate(lastDay.getUTCDate() - 1);
  ...
  for (let offset = -1; offset <= monthsAhead; offset++) {
    ...
    if (nfp) events.push({ date: toStr(nfp), time: '08:30', tz: 'America/New_York', scheduled: false, event: 'Nonfarm Payrolls', impact: 'High', forecast: '', previous: '', actual: '' });

    // CPI — typically 2nd Wednesday of the month (for prior month data)
    const cpi = nthWeekday(y, m, 3, 2);
    if (cpi) events.push({ date: toStr(cpi), time: '08:30', tz: 'America/New_York', scheduled: false, event: 'CPI (Consumer Price Index)', impact: 'High', forecast: '', previous: '', actual: '' });

    // PPI — day after CPI (Thursday), same week
    if (cpi) {
      const ppi = addDays(cpi, 1);
      events.push({ date: toStr(ppi), time: '08:30', tz: 'America/New_York', scheduled: false, event: 'PPI (Producer Price Index)', impact: 'Medium', forecast: '', previous: '', actual: '' });
    }

    // PCE — last Friday of the month
    const pce = lastWeekday(y, m, 5);
    if (pce) events.push({ date: toStr(pce), time: '08:30', tz: 'America/New_York', scheduled: false, event: 'PCE Price Index', impact: 'High', forecast: '', previous: '', actual: '' });
    ...
    if (retail) {
      ...
      if (toStr(retailActual) !== toStr(retail)) {
        events.push({ date: toStr(retailActual), time: '08:30', tz: 'America/New_York', scheduled: false, event: 'Retail Sales', impact: 'Medium', forecast: '', previous: '', actual: '' });
      }
    }

    // JOLTS — 2nd Tuesday of month
    const jolts = nthWeekday(y, m, 2, 2);
    if (jolts) events.push({ date: toStr(jolts), time: '10:00', tz: 'America/New_York', scheduled: false, event: 'JOLTS Job Openings', impact: 'Medium', forecast: '', previous: '', actual: '' });

    // Consumer Confidence — last Tuesday of month
    const cc = lastWeekday(y, m, 2);
    if (cc) events.push({ date: toStr(cc), time: '10:00', tz: 'America/New_York', scheduled: false, event: 'Consumer Confidence', impact: 'Medium', forecast: '', previous: '', actual: '' });
    ...
    if (claims) events.push({ date: toStr(claims), time: '08:30', tz: 'America/New_York', scheduled: false, event: 'Initial Jobless Claims', impact: 'Medium', forecast: '', previous: '', actual: '' });
  }
  ...
  return new Response(JSON.stringify({ source: 'dynamic_fallback', time_zone: 'America/New_York', scheduled: false, events: filtered }), { headers: corsHeaders });
}
