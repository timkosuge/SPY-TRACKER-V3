(function () {
  ...
  function render() {
    ...
    const vtext = T.verdict === 'expensive' ? 'CONTRACTS ARE EXPENSIVE' : T.verdict === 'cheap' ? 'CONTRACTS ARE CHEAP' : 'CONTRACTS ARE ORDINARILY PRICED';
    ...
    el.innerHTML = `
      <div class="panel" style="border-left:4px solid ${vcol};margin-bottom:12px;">
        ...
        <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:baseline;margin-bottom:6px;">
          ...
          <div><span style="font-family:'Orbitron',monospace;font-size:7px;color:var(--text3);">PRICED PER SESSION vs DELIVERED</span><br><span style="font-family:'Share Tech Mono',monospace;font-size:20px;color:var(--text);">${f2(T.expected_daily_move_pct)}%</span> <span style="font-size:10px;color:var(--text3);">against ${f2(T.delivered_daily_move_pct)}% median</span></div>
        </div>
