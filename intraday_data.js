...
(function () {
  ...
  // Opening hour: use or_break_dir + or_break_time to determine first-hour direction
  // If break happens early (within first ~30m), that IS the first-hour direction
  // If no break info, use initial_move_dir as proxy
  function computeOpenHour(data) {
    let up=0,dn=0,pcts=[];
    data.forEach(d=>{
      if(d.open_price==null||d.or_high==null||d.or_low==null) return;
      // Proxy for opening hour close: use or_break_dir if available, else initial_move_dir
      const dir=d.or_break_dir||d.initial_move_dir;
      if(!dir) return;
      // Estimate opening hour pct from OR range direction
      const proxy = dir==='UP'
        ? (d.or_high-d.open_price)/d.open_price*100
        : -(d.open_price-d.or_low)/d.open_price*100;
      pcts.push(proxy);
  ...
  // Session stat box (open hour / lunch / power hour)
  function sessionBox(title, timeLabel, upCount, dnCount, total, avgPct, spy, color) {
    ...
    const dolStr=spy&&avgPct!=null?' ($'+(spy*Math.abs(avgPct)/100).toFixed(2)+')':'';
    return '<div class="stat-box" style="padding:12px;text-align:center;">'
      ...
      +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:18px;color:'+color+'">'+fmtp(avgPct)+'</div>'
      +(spy&&avgPct!=null?'<div style="font-size:10px;color:var(--text3);">'+dolStr+'</div>':'')
  ...
  function tbar(label,count,maxCount,color,barWidth) {
    const bw=barWidth||110;
    const w=Math.round(count/maxCount*bw);
    return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">'
      +'<span style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:var(--text2);width:70px;text-align:right">'+label+'</span>'
      +'<div style="width:'+w+'px;height:9px;background:'+color+';border-radius:2px;"></div>'
  ...
  function renderIntraday() {
    ...
    function dol(pVal) {
      ...
      return ' <span style="color:var(--text3);font-size:0.82em">$'+(spy*Math.abs(pVal)/100).toFixed(2)+'</span>';
    }
    ...
    // Lunch stats (range-based — not directional)
    const lunchUp   = raw.filter(d=>d.power_hour_dir!=null&&d.lunch_range_pct!=null&&d.close_price>d.open_price).length;
    const lunchDn   = raw.filter(d=>d.power_hour_dir!=null&&d.lunch_range_pct!=null&&d.close_price<d.open_price).length;
    // Estimate avg lunch pct direction from overall day direction during lunch
    // We don't have a lunch close field; use avgLunch as range, sign from day direction proxy
    const lunchAvgPct = avgLunch||null; // range only, no sign

    // 5-min OR break times (ET->CT), only first 2.5 hrs of session: 9:30-12:00 ET
    const orBreak5 = {};
    raw.filter(d=>d.or_break_time).forEach(d=>{
      const[h,m]=d.or_break_time.split(':').map(Number);
      if(h*60+m < 9*60+30 || h*60+m > 12*60) return; // only 9:30-12:00 ET
      const m5=(Math.floor(m/5)*5);
      // ET->CT: h-1
      const ctH=h-1; const ctM=m5;
      const bucket=ctH+':'+String(ctM).padStart(2,'0');
      orBreak5[bucket]=(orBreak5[bucket]||0)+1;
    });
    // Sort chronologically, take top 12 by count
    ...
    const orBreak5Bars=orBreak5sorted.map(([t,c])=>{
      ...
      return tbar(label,c,maxORB5,'var(--cyan)',160);
    }).join('');
    ...
    const gapBars=Object.entries(gapB).map(([label,count])=>{
      const w=Math.round(count/maxGB*130);
      ...
      return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">'
        ...
        +'<div style="width:'+w+'px;height:12px;background:'+col+';border-radius:2px;opacity:0.85;"></div>'
...
// ── Session rhythm strip: OPEN HOUR | LUNCH | POWER HOUR ──
+'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:14px;">'
  ...
  +'<div class="stat-box" style="padding:12px;text-align:center;">'
    ...
    +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:18px;color:var(--text2)">'+(avgLunch!=null?fmt(avgLunch)+'%':'\u2014')+'</div>'
    +(spy&&avgLunch?'<div style="font-size:10px;color:var(--text3);">avg range $'+(spy*avgLunch/100).toFixed(2)+'</div>':'')
    +'<div style="font-size:10px;color:var(--text3);margin-top:8px;line-height:1.5;">Low vol compression window.<br>Tight range \u2192 watch for PM expansion.</div>'
  +'</div>'
...
+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">'
  ...
  +'<div class="stat-box" style="padding:14px;">'
    ...
    +'<div style="font-size:10px;color:var(--text3);margin-bottom:12px;">How often does an early directional move hold vs. reverse by end of day? All times CT.</div>'
  ...
  // Gap Analysis - bigger chart area
  +'<div class="stat-box" style="padding:14px;">'
...
// ── Row 2: OR Break Time Histogram (full width) ──
+'<div class="stat-box" style="padding:14px;margin-bottom:12px;">'
  +'<div class="es-section" style="margin-bottom:6px;">WHEN DOES THE OPENING RANGE BREAK? (8:30\u20139:00 CT) \u2014 5-MIN BUCKETS</div>'
  +'<div style="font-size:10px;color:var(--text3);margin-bottom:12px;">'
    +'The Opening Range (OR) is the high and low set in the first 30 minutes (8:30\u20139:00 CT). '
    +'An <strong style="color:var(--cyan);">\u201COR break\u201D</strong> is when price closes a 1-min bar <em>above the OR high</em> or <em>below the OR low</em> for the first time. '
    +'This chart shows what time that breakout happens most often. The earlier the break, the stronger the directional conviction for the day.'
  +'</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">'
    +'<div>'+(orBreak5Bars||'<div style="font-size:11px;color:var(--text3);">Not enough data</div>')+'</div>'
    +'<div style="font-size:11px;color:var(--text2);line-height:1.8;">'
      +'<div style="color:var(--cyan);font-size:10px;letter-spacing:1px;margin-bottom:8px;">KEY TAKEAWAYS</div>'
      +'OR breaks above <span style="color:var(--green)">'+Math.round(orBreakUp.length/n*100)+'%</span> of the time, '
      +'below <span style="color:var(--red)">'+Math.round(orBreakDown.length/n*100)+'%</span>.<br>'
      +'When it breaks up and holds: closes above OR high <span style="color:var(--green)">'+(orBreakUp.length?Math.round(orBreakUpHolds.length/orBreakUp.length*100):0)+'%</span> of days.<br>'
      +'When it breaks down and holds: closes below OR low <span style="color:var(--red)">'+(orBreakDown.length?Math.round(orBreakDownHolds.length/orBreakDown.length*100):0)+'%</span> of days.<br>'
      +'<br>'
...
// ── Row 3: Best/Worst 5-min windows ──
+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">'

  +'<div class="stat-box" style="padding:14px;">'
    +'<div class="es-section" style="margin-bottom:6px;">\ud83d\udfe2 STRONGEST 5-MIN WINDOWS (CT)</div>'
    +'<div style="font-size:10px;color:var(--text3);margin-bottom:8px;">Times of day most often producing the largest single 5-min up-moves</div>'
  ...
  +'<div class="stat-box" style="padding:14px;">'
    ...
    +'<div style="font-size:10px;color:var(--text3);margin-bottom:8px;">Times of day most often producing the largest single 5-min down-moves</div>'
