...
(function () {
  ...
  // ── Filter state ───────────────────────────────────────────────────────────
  let _lookback = 'all';   // 'all' | '2026'
  let _dow      = 'all';   // 'all' | 'Mon' | 'Tue' | 'Wed' | 'Thu' | 'Fri'
  ...
  function fbtn(label, active, onclick) {
    ...
    return '<button onclick="'+onclick+'" style="padding:3px 10px;font-family:\'Share Tech Mono\',monospace;font-size:10px;'
      +'background:'+bg+';color:'+fg+';border:1px solid '+bd+';border-radius:3px;cursor:pointer;">'+label+'</button>';
  }
  ...
  function renderIntraday() {
    ...
    raw.filter(d=>d.or_break_time).forEach(d=>{
      let [h,m]=d.or_break_time.split(':').map(Number); h-=1;
    ...
    const orbBars  =topORB.map(([t,c])=>{
      ...
      return tbar((h%12||12)+':'+min+' '+(h>=12?'PM':'AM'),c,maxORB2,'var(--cyan)');
    }).join('');
    ...
    const lbBtns=fbtn('ALL TIME',_lookback==='all',"window._intradaySetLookback('all')")
      +' '+fbtn('2026',_lookback==='2026',"window._intradaySetLookback('2026')");
    const dowBtns=['all','Mon','Tue','Wed','Thu','Fri'].map(d=>
      fbtn(d==='all'?'ALL':d.toUpperCase(),_dow===d,"window._intradaySetDOW('"+d+"')")
    ).join(' ');

    if(!n) {
      el.innerHTML='<div style="padding:40px;text-align:center;color:var(--text3);font-size:12px;">No sessions match this filter. Try a different combination.</div>';
      return;
    }
...
+'<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:10px;">'
  +'<div>'
    ...
    +'<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:5px;">'
      +'<span style="font-size:10px;color:var(--text3);width:60px;">LOOKBACK:</span>'+lbBtns
    +'</div>'
    +'<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">'
      +'<span style="font-size:10px;color:var(--text3);width:60px;">DAY:</span>'+dowBtns
    +'</div>'
  +'</div>'
...
+'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-bottom:14px;">'
  ...
  +kpi('OR BREAK UP',    n?Math.round(orBreakUp.length/n*100)+'%':'\u2014', null, 'holds above: '+(orBreakUp.length?Math.round(orBreakUpHolds.length/orBreakUp.length*100)+'%':'\u2014'))
  +kpi('OR BREAK DOWN',  n?Math.round(orBreakDown.length/n*100)+'%':'\u2014', null, 'holds below: '+(orBreakDown.length?Math.round(orBreakDownHolds.length/orBreakDown.length*100)+'%':'\u2014'))
  +kpi('AVG POWER HOUR', fmtp(avgPHpct), avgPHpct,  phUp.length+'\u2191 '+phDown.length+'\u2193'+dol(avgPHpct))
  +kpi('LUNCH RANGE',    avgLunch!=null?fmt(avgLunch)+'%':'\u2014', null, 'compression'+dol(avgLunch))
+'</div>'
...
+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">'
  ...
  +'<div class="stat-box" style="padding:14px;">'
    ...
    +'<div style="font-size:10px;color:var(--text3);letter-spacing:1px;margin-bottom:8px;">FIRST 15 MIN MOVE (8:30\u20138:44 CT) \u2014 '+rev15.total+' sessions</div>'
    ...
    +'<div style="font-size:10px;color:var(--text3);letter-spacing:1px;margin-bottom:8px;">FIRST 30 MIN MOVE (8:30\u20139:00 CT) \u2014 '+rev30.total+' sessions</div>'
...
+'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:12px;">'
  ...
  +'<div class="stat-box" style="padding:14px;">'
    ...
    +'<div style="font-size:11px;color:var(--text2);line-height:1.8;margin-bottom:10px;">'
      +'Avg width: <span style="color:var(--cyan)">'+(avgOR!=null?fmt(avgOR)+'%':'\u2014')+(spy&&avgOR?' ($'+(spy*avgOR/100).toFixed(2)+')':'')+'</span>.<br>'
      +'Breaks above <span style="color:var(--green)">'+Math.round(orBreakUp.length/n*100)+'%</span> of days'
      +' (closes above OR high: <span style="color:var(--green)">'+(orBreakUp.length?Math.round(orBreakUpHolds.length/orBreakUp.length*100):0)+'%</span>).<br>'
      +'Breaks below <span style="color:var(--red)">'+Math.round(orBreakDown.length/n*100)+'%</span> of days'
      +' (closes below OR low: <span style="color:var(--red)">'+(orBreakDown.length?Math.round(orBreakDownHolds.length/orBreakDown.length*100):0)+'%</span>).'
    +'</div>'
  ...
  +'<div class="stat-box" style="padding:14px;">'
    ...
    +'<div style="margin-top:4px;font-size:11px;color:var(--text2);line-height:1.7;">'
      ...
      +'Avg: <span style="color:'+((avgPHpct||0)>=0?'var(--green)':'var(--red)')+'">'+fmtp(avgPHpct)+(spy&&avgPHpct!=null?' ($'+(spy*Math.abs(avgPHpct)/100).toFixed(2)+')':'')+'</span>.'
    +'</div>'
  +'</div>'
