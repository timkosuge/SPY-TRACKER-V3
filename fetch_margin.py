...
the download once a day, over a connection that negotiates like a browser. Every attempt is logged with its result, so the site can say when a month could not be fetched
...
def _session():
    """A connection that negotiates like Chrome; FINRA refuses clients that do not."""
    from curl_cffi import requests as browser_requests
    return browser_requests.Session(impersonate="chrome")


def fetch():
    """FINRA's page, then its workbook on the same connection. Returns (rows or None, http status, what happened)."""
    try:
        s = _session()
        s.get(FINRA_PAGE, timeout=30)
        r = s.get(FINRA_XLSX, timeout=45, headers={"Referer": FINRA_PAGE})
    ...
    try:
        return parse_workbook(r.content), 200, "workbook fetched from FINRA"
    except Exception as e:
