(function () {
  ...
  const holdsMark = st => !st || st.holds == null ? '<span style="color:var(--text3);" title="too few sessions to split">·</span>' : st.holds ? '<span style="color:var(--green);" title="rate agrees in both halves of the sample">●</span>' : '<span style="color:var(--red);" title="the two halves of the sample disagree">●</span>';
  ...
  function render() {
    ...
    const logRows = D.log.map(r => `<tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
      ...
      <td style="padding:4px 8px;font-family:'Share Tech Mono',monospace;font-size:10px;color:var(--text2);">${r.prior_vix || '—'} · ${r.prior_dd || '—'}</td>
    ...
    const era = D.table_b[S.era] ? S.era : '3y';
    const cellKey = `${L.vix_bucket}|${L.dd_bucket}|${L.class}`;
    const tc = (D.table_b_cells && D.table_b_cells[era] && D.table_b_cells[era][cellKey]) || null;
    const tc3 = tc ? tc['3'] : null, tc5 = tc ? tc['5'] : null;
    const vixRow = D.table_b_regime[era].vix, ddRow = D.table_b_regime[era].dd;
    const regimeTable = (rows, keys, label) => `<table style="width:100%;border-collapse:collapse;font-size:11px;"><thead><tr>${[label, 'n', 'MEDIAN BEST, 3 SESSIONS', '≥1.5% IN 3', '≥1.5% IN 5', 'HOLDS'].map(h => `<th style="font-family:'Orbitron',monospace;font-size:7px;letter-spacing:1px;color:var(--text3);text-align:left;padding:5px 8px;border-bottom:1px solid var(--border);">${h}</th>`).join('')}</tr></thead><tbody>${keys.map(k => { const r3 = rows[k]['3'], r5 = rows[k]['5']; const on = k === L.vix_bucket || k === L.dd_bucket; return `<tr style="${on ? 'background:rgba(0,204,255,0.08);' : ''}"><td style="padding:5px 8px;font-family:'Share Tech Mono',monospace;color:var(--text);">${k}</td><td style="padding:5px 8px;font-family:'Share Tech Mono',monospace;color:var(--text3);">${r3.n.toLocaleString()}</td><td style="padding:5px 8px;font-family:'Share Tech Mono',monospace;color:var(--text);">${r3.n >= floor ? f2(r3.median_best) + '%' : '—'}</td><td style="padding:5px 8px;">${rateCell(r3.ge_150, floor)}</td><td style="padding:5px 8px;">${rateCell(r5.ge_150, floor)}</td><td style="padding:5px 8px;">${holdsMark(r3.stability)}</td></tr>`; }).join('')}</tbody></table>`;
    ...
    el.innerHTML = `
      ...
      <div class="panel" style="margin-bottom:12px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;">
          ${[['vix', D.vix_buckets, 'VIX AT PRIOR CLOSE · WIDE OR'], ['dd', D.dd_buckets, 'FROM 20-SESSION HIGH · WIDE OR']].map(([key, keys, label]) => `<div><table style="width:100%;border-collapse:collapse;font-size:11px;"><thead><tr>${[label, 'n', 'MEDIAN BEST', '≥0.75%', 'HOLDS'].map(h => `<th style="font-family:'Orbitron',monospace;font-size:7px;letter-spacing:1px;color:var(--text3);text-align:left;padding:5px 8px;border-bottom:1px solid var(--border);">${h}</th>`).join('')}</tr></thead><tbody>${keys.map(k => { const c = D.table_a_regime[key][k].wide; const on = k === (key === 'vix' ? L.vix_bucket : L.dd_bucket); return `<tr style="${on ? 'background:rgba(0,204,255,0.08);' : ''}"><td style="padding:5px 8px;font-family:'Share Tech Mono',monospace;color:var(--text);">${k}</td><td style="padding:5px 8px;font-family:'Share Tech Mono',monospace;color:var(--text3);">${c.n}</td><td style="padding:5px 8px;font-family:'Share Tech Mono',monospace;color:var(--text);">${c.n >= floor ? f2(c.median_best) + '%' : '—'}</td><td style="padding:5px 8px;">${rateCell(c.ge_075, floor)}</td><td style="padding:5px 8px;">${holdsMark(c.stability)}</td></tr>`; }).join('')}</tbody></table></div>`).join('')}
        </div>
        <div style="font-size:10px;color:var(--text3);margin-top:6px;">On wide-opening-range days only: the regime at the prior close changes how far the move goes. HOLDS marks agreement between the two halves of the intraday sample.</div>
      </div>
      <div class="panel" style="margin-bottom:12px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;">
          <div>${regimeTable(vixRow, D.vix_buckets, 'VIX AT THE CLOSE')}</div>
          <div>${regimeTable(ddRow, D.dd_buckets, 'CLOSE VS 20-SESSION HIGH')}</div>
        </div>
        <div style="font-size:10px;color:var(--text3);margin-top:6px;">The two conditions that add to the day's range. HOLDS: ● green — the rate agrees within its interval on both halves of the sample; ● red — the halves disagree; · — too few to split. Highlighted rows are the regime at the latest close. VIX closes from the site's own table, ${D.vix_coverage.sessions.toLocaleString()} sessions through ${fmtDate(D.vix_coverage.last, 'short')}.</div>
      </div>
      <div class="panel" style="margin-bottom:12px;">
        ...
        <div style="font-size:12px;color:var(--text2);line-height:1.8;">
          0. At the close, read the regime. VIX ${L.vix_bucket ? L.vix_bucket : '—'} and ${L.dd_bucket ? L.dd_bucket : '—'} of the 20-session high today. Across every era, VIX under 15 with the market within 2% of its high is the corner where 1.5% moves are rarest; VIX over 20 or a market more than 2% off its high is where they are common. The regime rows above carry today's numbers.<br>
