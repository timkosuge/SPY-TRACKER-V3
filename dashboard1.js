...
const GROUP_TABS = {
  ...
  derivatives: ['options','gex','gex-intraday','options-env','wem','volatility'],
...
function _switchPanelOnly(id) {
  ...
  if(id==='sentiment') { if(typeof renderCotPositioning==='function') renderCotPositioning(); if(typeof renderSmartDumb==='function') renderSmartDumb(); }
  ...
  el.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
      ${wem?(()=>{
        ...
        const openAnchor = mktOpen ? (todayOpen || cur || midP) : lastClose;
...
function renderOptions(md){
  ...
  const nearestLabel = nearestExp===etToday() ? '0DTE' : nearestExp===thisFriStr ? 'WEEKLY OPEX' : nearestExp===monthlyOpexStr ? 'MONTHLY OPEX' : `${nearestExpDow} expiry`;
  ...
  if(banner){
    ...
    const bannerColor = nearestExp===etToday()?'#ff3355':nearestExp===thisFriStr?'#ffcc00':nearestExp===monthlyOpexStr?'#ff8800':'var(--cyan)';
