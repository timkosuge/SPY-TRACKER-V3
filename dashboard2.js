...
function renderOverview(md){
  ...
  el.innerHTML=`
    ...
    <!-- ROW 4: S&P 500 HEATMAP -->
    <div style="margin-top:12px;">
      <div style="font-family:'Orbitron',monospace;font-size:9px;letter-spacing:2px;color:#00ccff;margin-bottom:6px;padding-bottom:3px;border-bottom:1px solid #00ccff22;">⬡ S&P 500 HEATMAP</div>
    ...
    <!-- ROW 5: ROTATION DETECTOR -->
    <div style="margin-top:12px;" id="rotationDetectorPanel"></div>
  `;

  // Snapshot + render rotation detector after overview HTML is set
  _rotationSnapshotAndRender(md);
}
...
// ─────────────────────────────────────────────────────────────────────────────
// ROTATION DETECTOR
// Accumulates daily sector snapshots in localStorage, builds rolling history,
// detects multi-timeframe rotation patterns, and renders a narrative summary.
// ─────────────────────────────────────────────────────────────────────────────

const ROTATION_CACHE_KEY = 'spy_rotation_history_v2';
const ROTATION_MAX_DAYS  = 60; // keep 60 days of snapshots

// Sectors in risk-appetite order: offensive → defensive
const ROT_SECTORS = [
  { sym:'XLK',  name:'Technology',    cat:'offensive',  spw:32 },
  { sym:'XLY',  name:'Cons Disc',     cat:'offensive',  spw:11 },
  { sym:'XLC',  name:'Comm Svcs',     cat:'offensive',  spw:9  },
  { sym:'XLI',  name:'Industrials',   cat:'cyclical',   spw:8  },
  { sym:'XLF',  name:'Financials',    cat:'cyclical',   spw:13 },
  { sym:'XLB',  name:'Materials',     cat:'cyclical',   spw:3  },
  { sym:'XLE',  name:'Energy',        cat:'cyclical',   spw:4  },
  { sym:'XLV',  name:'Health Care',   cat:'defensive',  spw:12 },
  { sym:'XLP',  name:'Cons Staples',  cat:'defensive',  spw:6  },
  { sym:'XLU',  name:'Utilities',     cat:'defensive',  spw:2  },
  { sym:'XLRE', name:'Real Estate',   cat:'defensive',  spw:2  },
];

const ROT_RATIOS = [
  { key:'qqq_iwm',  symA:'QQQ',  symB:'IWM',  name:'Growth/Small',  riskDir:1  },
  { key:'xlk_xlv',  symA:'XLK',  symB:'XLV',  name:'Tech/Health',   riskDir:1  },
  { key:'xlp_xly',  symA:'XLP',  symB:'XLY',  name:'Staples/Disc',  riskDir:-1 },
  { key:'tlt_spy',  symA:'TLT',  symB:'SPY',  name:'Bonds/SPY',     riskDir:-1 },
  { key:'hyg_lqd',  symA:'HYG',  symB:'LQD',  name:'HY/IG Credit',  riskDir:1  },
];

function _rotationLoadHistory() {
  try {
...
function _rotationSaveHistory(hist) {
  try { localStorage.setItem(ROTATION_CACHE_KEY, JSON.stringify(hist)); } catch(e) {}
}

function _rotationBuildSnapshot(md) {
  const q   = md.quotes || {};
  const etToday = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/New_York' }).format(new Date());
  const snap = { date: etToday, sectors: {}, ratios: {}, vix: null, spy: null };

  // Sector % changes
  for (const s of ROT_SECTORS) {
    const v = q[s.sym]?.pct_change;
    if (v != null) snap.sectors[s.sym] = parseFloat(v.toFixed(4));
  }

  // Ratio prices (for computing rolling ratio momentum)
  for (const r of ROT_RATIOS) {
    const a = q[r.symA]?.price, b = q[r.symB]?.price;
    if (a && b) snap.ratios[r.key] = parseFloat((a / b).toFixed(6));
  }

  snap.vix = q['^VIX']?.price ?? null;
  snap.spy = q['SPY']?.pct_change ?? null;

  return snap;
}

function _rotationSnapshotAndRender(md) {
  const el = document.getElementById('rotationDetectorPanel');
  if (!el) return;

  // Load history, upsert today's snapshot
  const hist = _rotationLoadHistory();
  const snap = _rotationBuildSnapshot(md);

  const existingIdx = hist.findIndex(h => h.date === snap.date);
  if (existingIdx >= 0) {
    hist[existingIdx] = snap; // update with latest intraday data
  } else {
    hist.unshift(snap);
  }

  // Trim to max days, sort newest-first
  hist.sort((a,b) => b.date.localeCompare(a.date));
  while (hist.length > ROTATION_MAX_DAYS) hist.pop();
  _rotationSaveHistory(hist);

  if (hist.length < 2) {
    el.innerHTML = `<div class="panel" style="padding:12px;font-size:12px;color:var(--text3);">Rotation detector is building history — check back after a few sessions.</div>`;
    return;
  }

  _rotationRender(el, hist, md);
}

function _rotationRender(el, hist, md) {
  const q = md.quotes || {};

  // ── Compute rolling momentum for each sector over N days ──────────────────
  function sectorMomentum(sym, days) {
    // Sum of daily % changes over the last N snapshots
    const slc = hist.slice(0, Math.min(days, hist.length));
    let sum = 0, count = 0;
    for (const h of slc) {
      const v = h.sectors[sym];
      if (v != null) { sum += v; count++; }
    }
    return count > 0 ? sum : null;
  }

  function ratioMomentum(key, days) {
    // % change in ratio from N days ago to today
    const now  = hist[0]?.ratios[key];
    const then = hist[Math.min(days, hist.length - 1)]?.ratios[key];
    if (!now || !then) return null;
    return ((now - then) / then) * 100;
  }

  // Compute momentum across timeframes for each sector
  const sectorData = ROT_SECTORS.map(s => ({
    ...s,
    today: hist[0]?.sectors[s.sym] ?? null,
    m3:    sectorMomentum(s.sym, 3),
    m5:    sectorMomentum(s.sym, 5),
    m10:   sectorMomentum(s.sym, 10),
    m20:   sectorMomentum(s.sym, 20),
  }));

  // Rank sectors by 5-day momentum
  const ranked5 = [...sectorData]
    .filter(s => s.m5 != null)
    .sort((a,b) => b.m5 - a.m5);

  // ── Rotation scoring ───────────────────────────────────────────────────────
  // For each timeframe, score offensive vs defensive leadership
  function rotScore(days) {
    const key = days === 3 ? 'm3' : days === 5 ? 'm5' : days === 10 ? 'm10' : 'm20';
    const offensive = sectorData.filter(s => s.cat === 'offensive' && s[key] != null);
    const defensive = sectorData.filter(s => s.cat === 'defensive' && s[key] != null);
    const cyclical  = sectorData.filter(s => s.cat === 'cyclical'  && s[key] != null);
    if (!offensive.length || !defensive.length) return null;
    const avgOff = offensive.reduce((a,b) => a + b[key], 0) / offensive.length;
    const avgDef = defensive.reduce((a,b) => a + b[key], 0) / defensive.length;
    const avgCyc = cyclical.reduce((a,b) => a + b[key], 0) / (cyclical.length || 1);
    return { avgOff, avgDef, avgCyc, spread: avgOff - avgDef };
  }
  ...
  // Ratio momentum
  const ratioData = ROT_RATIOS.map(r => ({
    ...r,
    m5:  ratioMomentum(r.key, 5),
    m10: ratioMomentum(r.key, 10),
    m20: ratioMomentum(r.key, 20),
  }));

  // ── Pattern detection ──────────────────────────────────────────────────────
  const patterns = [];
  const dayCount = hist.length;

  // Helper: how many of last N days did offensive outperform defensive?
  function offLeadDays(n) {
    let count = 0;
    for (let i = 0; i < Math.min(n, hist.length); i++) {
      const h = hist[i];
      const off = ROT_SECTORS.filter(s => s.cat==='offensive').map(s => h.sectors[s.sym]).filter(v => v != null);
      const def = ROT_SECTORS.filter(s => s.cat==='defensive').map(s => h.sectors[s.sym]).filter(v => v != null);
      if (!off.length || !def.length) continue;
      const avgO = off.reduce((a,b)=>a+b,0)/off.length;
      const avgD = def.reduce((a,b)=>a+b,0)/def.length;
      if (avgO > avgD) count++;
    }
    return count;
  }

  function defLeadDays(n) { return Math.min(n, hist.length) - offLeadDays(n); }

  const offLead5 = offLeadDays(5), defLead5 = defLeadDays(5);
  const offLead10 = offLeadDays(10), defLead10 = defLeadDays(10);

  // VIX trend
  const vixNow   = hist[0]?.vix, vix5ago = hist[Math.min(5,hist.length-1)]?.vix;
  const vixTrend = vixNow && vix5ago ? vixNow - vix5ago : null;

  // Bond trend (TLT ratio)
  const bondMom5 = ratioData.find(r=>r.key==='tlt_spy')?.m5;

  // Credit spread (HYG/LQD)
  const creditMom5 = ratioData.find(r=>r.key==='hyg_lqd')?.m5;

  // SPY cumulative 5d + 10d
  const spy5  = hist.slice(0,Math.min(5,hist.length)).reduce((a,h)=>a+(h.spy??0),0);
  const spy10 = hist.slice(0,Math.min(10,hist.length)).reduce((a,h)=>a+(h.spy??0),0);

  // ── Pattern rules ──────────────────────────────────────────────────────────

  // 1. Sustained risk-off rotation
  if (defLead5 >= 4 && (scores.d5?.spread ?? 0) < -0.5) {
    patterns.push({
      strength: defLead5 >= 5 ? 'STRONG' : 'MODERATE',
      color: '#ff3355',
      title: 'Risk-Off Rotation Building',
      pieces: [
        `Defensives (${['XLV','XLP','XLU','XLRE'].filter(s=>sectorData.find(x=>x.sym===s)?.m5!=null&&sectorData.find(x=>x.sym===s).m5>0).join(', ')||'—'}) leading for ${defLead5}/5 days`,
        vixTrend!=null ? `VIX ${vixTrend>0?'rising':'falling'} ${Math.abs(vixTrend).toFixed(1)} pts over 5d` : null,
        bondMom5!=null ? `Bonds vs SPY: ${bondMom5>0?'outperforming':'underperforming'} (${bondMom5>0?'+':''}${bondMom5.toFixed(2)}% 5d)` : null,
        `Offensive/Defensive spread: ${(scores.d5?.spread??0).toFixed(2)}% 5d`,
      ].filter(Boolean),
      implication: 'Money moving to safety. Watch for SPY weakness if this persists into week 2.',
    });
  }

  // 2. Sustained risk-on rotation
  if (offLead5 >= 4 && (scores.d5?.spread ?? 0) > 0.5) {
    patterns.push({
      strength: offLead5 >= 5 ? 'STRONG' : 'MODERATE',
      color: '#00ff88',
      title: 'Risk-On Rotation Building',
      pieces: [
        `Offensives (${['XLK','XLY','XLC'].filter(s=>sectorData.find(x=>x.sym===s)?.m5!=null&&sectorData.find(x=>x.sym===s).m5>0).join(', ')||'—'}) leading for ${offLead5}/5 days`,
        creditMom5!=null ? `HY credit ${creditMom5>0?'tightening':'widening'} (${creditMom5>0?'+':''}${creditMom5.toFixed(2)}% 5d vs IG)` : null,
        `Offensive/Defensive spread: +${(scores.d5?.spread??0).toFixed(2)}% 5d`,
      ].filter(Boolean),
      implication: 'Broad participation and risk appetite. Pullbacks likely shallow.',
    });
  }

  // 3. Divergence: short-term flip vs medium-term trend
  if (scores.d3 && scores.d10) {
    const flip = Math.sign(scores.d3.spread) !== Math.sign(scores.d10.spread);
    if (flip) {
      const d3Dir = scores.d3.spread > 0 ? 'risk-on' : 'risk-off';
      const d10Dir = scores.d10.spread > 0 ? 'risk-on' : 'risk-off';
      patterns.push({
        strength: 'WATCH',
        color: '#ffcc00',
        title: 'Short/Medium-Term Rotation Divergence',
        pieces: [
          `Last 3 days: ${d3Dir} (spread ${scores.d3.spread.toFixed(2)}%)`,
          `Last 10 days: ${d10Dir} (spread ${scores.d10.spread.toFixed(2)}%)`,
          `Possible ${d3Dir==='risk-on'?'relief rally within downtrend':'distribution within uptrend'}`,
        ],
        implication: `Short-term move opposing medium-term trend. High probability of mean reversion toward ${d10Dir}.`,
      });
    }
  }

  // 4. Sector leadership narrowing (only 1-2 sectors driving SPY)
  if (ranked5.length >= 5) {
    const top2Avg = (ranked5[0].m5 + ranked5[1].m5) / 2;
    const bot3Avg = (ranked5[ranked5.length-1].m5 + ranked5[ranked5.length-2].m5 + ranked5[ranked5.length-3].m5) / 3;
    if (top2Avg - bot3Avg > 3.0) {
      patterns.push({
        strength: 'WATCH',
        color: '#ff8800',
        title: 'Narrow Leadership — Breadth Warning',
        pieces: [
          `${ranked5[0].name} +${ranked5[0].m5.toFixed(2)}%, ${ranked5[1].name} +${ranked5[1].m5.toFixed(2)}% driving move`,
          `Bottom 3 sectors averaging ${bot3Avg.toFixed(2)}% 5d`,
          `Leadership gap: ${(top2Avg-bot3Avg).toFixed(2)}%`,
        ],
        implication: 'Narrow advances historically precede corrections. Watch for broadening or breakdown.',
      });
    }
  }

  // 5. Cyclical confirmation (cyclicals + offensives both leading)
  if (scores.d5 && scores.d10) {
    const cycNow  = scores.d5.avgCyc, offNow  = scores.d5.avgOff;
    const cyc10   = scores.d10.avgCyc, off10   = scores.d10.avgOff;
    if (cycNow > 0 && offNow > 0 && cyc10 > 0 && off10 > 0) {
      patterns.push({
        strength: 'MODERATE',
        color: '#00ccff',
        title: 'Cyclical + Offensive Alignment',
        pieces: [
          `Cyclicals 5d: +${scores.d5.avgCyc.toFixed(2)}%, 10d: +${scores.d10.avgCyc.toFixed(2)}%`,
          `Offensives 5d: +${scores.d5.avgOff.toFixed(2)}%, 10d: +${scores.d10.avgOff.toFixed(2)}%`,
          `Broad economic expansion signal across both timeframes`,
        ],
        implication: 'Sustained alignment of growth and cyclicals = healthy bull market internals.',
      });
    }
  }

  // 6. VIX compression while defensives lead (complacency risk)
  if (vixNow && vixNow < 16 && defLead5 >= 3 && (scores.d5?.avgDef ?? 0) > 0) {
    patterns.push({
      strength: 'WATCH',
      color: '#8855ff',
      title: 'VIX Compression + Defensive Creep',
      pieces: [
        `VIX at ${vixNow.toFixed(1)} — low volatility`,
        `Defensives quietly outperforming for ${defLead5}/5 days`,
        `Smart money may be rotating while surface appears calm`,
      ],
      implication: 'Low VIX masking stealth defensive rotation. Classic late-cycle positioning.',
    });
  }
  ...
  if (!patterns.length) {
    patterns.push({
      strength: 'NEUTRAL',
      color: '#606080',
      title: 'No Clear Rotation Pattern',
      pieces: [`${dayCount} days of history · monitoring for developing signals`],
      implication: 'Sector performance mixed across timeframes. No dominant rotation thesis.',
    });
  }

  // ── Rank sectors by 5d for the bar chart ──────────────────────────────────
  const barMax = Math.max(...ranked5.map(s => Math.abs(s.m5 ?? 0)), 1);
  ...
  const catColor = cat => cat==='offensive'?'#00ff88':cat==='cyclical'?'#ffcc00':'#00ccff';

  const f2 = v => v != null ? (v >= 0 ? '+' : '') + v.toFixed(2) + '%' : '—';
  const tfColor = v => v == null ? 'var(--text3)' : v > 0.3 ? '#00ff88' : v < -0.3 ? '#ff3355' : 'var(--text2)';
  ...
  <div class="panel" style="margin-top:0;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid var(--border);">
      <div style="font-family:'Orbitron',monospace;font-size:9px;letter-spacing:2px;color:#8855ff;">⬡ ROTATION DETECTOR <span style="font-weight:normal;color:var(--text3);font-size:8px;letter-spacing:0;">${dayCount} day${dayCount!==1?'s':''} of history</span></div>
      <div style="font-size:9px;color:var(--text3);font-family:'Share Tech Mono',monospace;">Updated: ${hist[0]?.date || '—'} · <span style="color:${hist.length>=10?'#00ff88':'#ffcc00'}">${hist.length>=10?'Full signal mode':'Building... (need 10d for full signals)'}</span></div>
    </div>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">

      <!-- LEFT: Pattern signals -->
      <div>
        <div style="font-family:'Orbitron',monospace;font-size:8px;color:var(--text3);letter-spacing:1px;margin-bottom:8px;">DETECTED PATTERNS</div>
        ${patterns.map(p => `
          <div style="margin-bottom:10px;padding:10px 12px;background:${p.color}10;border:1px solid ${p.color}33;border-left:4px solid ${p.color};border-radius:0 4px 4px 0;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
              <span style="font-family:'Orbitron',monospace;font-size:8px;background:${p.color}25;color:${p.color};padding:2px 6px;border-radius:2px;letter-spacing:1px;">${p.strength}</span>
              <span style="font-family:'Orbitron',monospace;font-size:9px;color:${p.color};">${p.title}</span>
            </div>
            <div style="margin-bottom:6px;">
              ${p.pieces.map(piece => `
                <div style="display:flex;align-items:flex-start;gap:6px;margin-bottom:2px;">
                  <span style="color:${p.color};font-size:10px;flex-shrink:0;margin-top:1px;">·</span>
                  <span style="font-size:11px;color:var(--text2);line-height:1.4;">${piece}</span>
                </div>`).join('')}
            </div>
            <div style="font-size:10px;color:var(--text3);border-top:1px solid ${p.color}22;padding-top:5px;margin-top:4px;font-style:italic;">${p.implication}</div>
          </div>`).join('')}

        <!-- Ratio momentum summary -->
        <div style="margin-top:10px;">
          <div style="font-family:'Orbitron',monospace;font-size:8px;color:var(--text3);letter-spacing:1px;margin-bottom:6px;">KEY RATIOS — MOMENTUM</div>
          ${ratioData.filter(r => r.m5!=null).map(r => {
            const riskSignal = r.m5 * r.riskDir;
            const rc = riskSignal > 0.3 ? '#00ff88' : riskSignal < -0.3 ? '#ff3355' : 'var(--text2)';
            return `<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid var(--border);">
              <span style="font-size:11px;color:var(--text3);">${r.name}</span>
              <div style="display:flex;gap:12px;">
                <span style="font-family:'Share Tech Mono',monospace;font-size:11px;color:${tfColor(r.m5)};">${f2(r.m5)} <span style="color:var(--text3);font-size:9px;">5d</span></span>
                <span style="font-family:'Share Tech Mono',monospace;font-size:11px;color:${tfColor(r.m10)};">${f2(r.m10)} <span style="color:var(--text3);font-size:9px;">10d</span></span>
                <span style="font-family:'Share Tech Mono',monospace;font-size:11px;color:${rc};">${riskSignal>0.3?'▲ RISK-ON':riskSignal<-0.3?'▼ RISK-OFF':'— NEUTRAL'}</span>
              </div>
            </div>`;
          }).join('')}
        </div>
      ...
      <!-- RIGHT: Sector momentum bars + timeframe table -->
      <div>
        <div style="font-family:'Orbitron',monospace;font-size:8px;color:var(--text3);letter-spacing:1px;margin-bottom:8px;">SECTOR MOMENTUM — 5-DAY RANK</div>
        ${ranked5.map((s,i) => {
          const barW = Math.abs(s.m5 ?? 0) / barMax * 100;
          const bc = catColor(s.cat);
          const isPos = (s.m5 ?? 0) >= 0;
          return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;">
            <div style="font-family:'Orbitron',monospace;font-size:8px;color:${bc};width:32px;text-align:right;flex-shrink:0;">${s.sym}</div>
            <div style="flex:1;height:18px;background:var(--bg2);border-radius:2px;overflow:hidden;position:relative;">
              <div style="position:absolute;${isPos?'left:0':'right:0'};top:0;width:${barW}%;height:100%;background:${isPos?'#00ff8866':'#ff335566'};border-radius:2px;"></div>
              <div style="position:absolute;left:6px;top:50%;transform:translateY(-50%);font-family:'Share Tech Mono',monospace;font-size:10px;color:${isPos?'#00ff88':'#ff3355'};">${f2(s.m5)}</div>
            </div>
            <div style="font-family:'Orbitron',monospace;font-size:7px;color:${bc};width:52px;flex-shrink:0;">${s.cat}</div>
          </div>`;
        }).join('')}

        <!-- Timeframe comparison table -->
        <div style="margin-top:14px;">
          <div style="font-family:'Orbitron',monospace;font-size:8px;color:var(--text3);letter-spacing:1px;margin-bottom:6px;">SECTOR MOMENTUM — ALL TIMEFRAMES</div>
          <div style="overflow-x:auto;">
          <table style="width:100%;border-collapse:collapse;min-width:340px;">
            <colgroup><col style="width:52px"><col style="width:80px">${tfCols.map(()=>'<col style="width:60px">').join('')}</colgroup>
            <thead><tr style="border-bottom:1px solid var(--border);">
              <th style="font-family:'Orbitron',monospace;font-size:7px;color:var(--text3);padding:3px 4px;text-align:left;letter-spacing:1px;">SYM</th>
              <th style="font-family:'Orbitron',monospace;font-size:7px;color:var(--text3);padding:3px 4px;text-align:left;letter-spacing:1px;">SECTOR</th>
              ${tfCols.map(t=>`<th style="font-family:'Orbitron',monospace;font-size:7px;color:var(--text3);padding:3px 4px;text-align:right;letter-spacing:1px;">${t.label}</th>`).join('')}
            </tr></thead>
            <tbody>
              ${sectorData.map(s => {
                const bc2 = catColor(s.cat);
                return `<tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
                  <td style="font-family:'Orbitron',monospace;font-size:8px;color:${bc2};padding:3px 4px;">${s.sym}</td>
                  <td style="font-size:9px;color:var(--text3);padding:3px 4px;">${s.name}</td>
                  ${tfCols.map(t=>`<td style="font-family:'Share Tech Mono',monospace;font-size:10px;color:${tfColor(s[t.key])};padding:3px 4px;text-align:right;">${f2(s[t.key])}</td>`).join('')}
                </tr>`;
              }).join('')}
              <!-- Summary rows -->
              <tr style="border-top:2px solid rgba(255,255,255,0.08);">
                <td colspan="2" style="font-family:'Orbitron',monospace;font-size:7px;color:#00ff88;padding:4px 4px 2px;">OFF AVG</td>
                ${tfCols.map(t => {
                  const v = scores[t.key==='m3'?'d3':t.key==='m5'?'d5':t.key==='m10'?'d10':'d20']?.avgOff ?? null;
                  return `<td style="font-family:'Share Tech Mono',monospace;font-size:10px;color:${tfColor(v)};padding:4px 4px 2px;text-align:right;">${f2(v)}</td>`;
                }).join('')}
              </tr>
              <tr>
                <td colspan="2" style="font-family:'Orbitron',monospace;font-size:7px;color:#00ccff;padding:2px 4px;">DEF AVG</td>
                ${tfCols.map(t => {
                  const v = scores[t.key==='m3'?'d3':t.key==='m5'?'d5':t.key==='m10'?'d10':'d20']?.avgDef ?? null;
                  return `<td style="font-family:'Share Tech Mono',monospace;font-size:10px;color:${tfColor(v)};padding:2px 4px;text-align:right;">${f2(v)}</td>`;
                }).join('')}
              </tr>
              <tr>
                <td colspan="2" style="font-family:'Orbitron',monospace;font-size:7px;color:#8855ff;padding:2px 4px 4px;">OFF-DEF</td>
                ${tfCols.map(t => {
                  const v = scores[t.key==='m3'?'d3':t.key==='m5'?'d5':t.key==='m10'?'d10':'d20']?.spread ?? null;
                  return `<td style="font-family:'Share Tech Mono',monospace;font-size:10px;color:${tfColor(v)};padding:2px 4px 4px;text-align:right;font-weight:bold;">${f2(v)}</td>`;
                }).join('')}
              </tr>
            </tbody>
          </table>
        ...
        <div style="font-size:9px;color:var(--text3);margin-top:8px;">
          <span style="color:#00ff88;">■</span> Offensive &nbsp;
          <span style="color:#ffcc00;">■</span> Cyclical &nbsp;
          <span style="color:#00ccff;">■</span> Defensive &nbsp;·&nbsp;
          Accumulates daily snapshots · ${ROTATION_MAX_DAYS}d max history
        </div>
      </div>
